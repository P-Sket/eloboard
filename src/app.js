...
const el = {
  ...
  eloSeasonHonors: document.querySelector("#elo-season-honors"),
  gslSeasonHonors: document.querySelector("#gsl-season-honors"),
  clanwarSummary: document.querySelector("#clanwar-summary"),
  clanwarByClan: document.querySelector("#clanwar-by-clan"),
  clanwarPlayers: document.querySelector("#clanwar-players"),
  clanwarLog: document.querySelector("#clanwar-log"),
  ...
  opponentClanList: document.querySelector("#opponent-clan-list"),
  ...
  clanwarBatch: document.querySelector("#clanwar-batch"),
  clanwarOpponentClan: document.querySelector("#clanwar-opponent-clan"),
  clanwarSets: document.querySelector("#clanwar-sets"),
  addClanwarSet: document.querySelector("#add-clanwar-set"),
...
let matches = [];
let maps = [];
let majorResults = { eloSeasons: [], gslSeasons: [] };
let opponentClans = [];
let clanWarMatches = [];
let standingsByMode = {
  normal: [],
...
async function init() {
  const [playerData, matchData, mapData, majorResultData, opponentClanData, clanWarData] = await loadData();

  players = playerData;
  maps = mapData;
  majorResults = normalizeMajorResults(majorResultData);
  opponentClans = normalizeOpponentClans(opponentClanData);
  clanWarMatches = normalizeClanWarMatches(clanWarData);
...
async function loadData() {
  if (CONFIG.dataSource === "google-sheets" && CONFIG.googleSheetsWebAppUrl) {
    try {
      ...
      const [majorResultData, opponentClanData, clanWarData] = await Promise.all([
        fetchOptionalSheetData({ action: "majorResults" }, { eloSeasons: [], gslSeasons: [] }),
        fetchOptionalSheetData({ action: "opponentClans" }, []),
        fetchOptionalSheetData({ action: "clanWarMatches" }, [])
      ]);
      return [sheetPlayers.players || [], sheetMatches.matches || [], mapsFromSheet || [], majorResultData, opponentClanData, clanWarData];
    } catch (error) {
      console.warn("Google Sheets data load failed.", error);
      const [playerData, matchData, mapData] = await loadLocalCoreData();
      return [playerData, matchData, mapData, { eloSeasons: [], gslSeasons: [] }, [], []];
    }
  }

  return [
    ...await loadLocalCoreData(),
    { eloSeasons: [], gslSeasons: [] },
    [],
    []
  ];
}

function loadLocalCoreData() {
  return Promise.all([
...
async function fetchOptionalSheetData(params, fallback) {
  try {
    return await fetchJsonp(CONFIG.googleSheetsWebAppUrl, params);
  } catch (error) {
    console.warn("Optional Google Sheets data load failed.", params.action, error);
    return fallback;
  }
}
...
function normalizeMatches(matchList) {
  return matchList.map((match, index) => ({
    ...match,
    sourceIndex: getMatchSourceIndex(match, index),
...
function normalizeMajorResults(data) {
  return {
    eloSeasons: normalizeHonorSeasons(data?.eloSeasons),
    gslSeasons: normalizeHonorSeasons(data?.gslSeasons)
  };
}

function normalizeHonorSeasons(seasons) {
  if (!Array.isArray(seasons)) return [];

  return seasons.map((season) => ({
    ...season,
    results: Array.isArray(season.results)
      ? season.results.map((result) => ({
        ...result,
        rank: Number(result.rank),
        playerId: canonicalPlayerId(result.playerId)
      }))
      : []
  }));
}

function normalizeOpponentClans(data) {
  const clans = Array.isArray(data?.clans) ? data.clans : Array.isArray(data) ? data : [];
  return clans
    .map((entry) => typeof entry === "string" ? entry : entry?.clan)
    .map((clan) => `${clan || ""}`.trim())
    .filter(Boolean);
}

function normalizeClanWarMatches(data) {
  const matchList = Array.isArray(data?.matches) ? data.matches : Array.isArray(data) ? data : [];
  return matchList
    .map((match, index) => ({
      ...match,
      sourceIndex: getMatchSourceIndex(match, index),
      seriesId: `${match.seriesId || ""}`.trim(),
      date: normalizeDateString(match.date),
      opponentClan: `${match.opponentClan || ""}`.trim(),
      ourPlayer: canonicalPlayerId(match.ourPlayer),
      opponentPlayer: `${match.opponentPlayer || ""}`.trim(),
      opponentRace: normalizeRace(match.opponentRace),
      result: match.result === "L" ? "L" : "W"
    }))
    .filter((match) => match.date && match.opponentClan && match.ourPlayer && match.opponentPlayer);
}

function normalizeRace(race) {
  const value = `${race || ""}`.trim();
  return RACES.find((entry) => entry.toLowerCase() === value.toLowerCase()) || value;
}

function getMatchSourceIndex(match, fallback = 0) {
  const value = Number(match.sourceIndex ?? match.rowIndex ?? match.rowNumber);
...
function buildStandings(playerList, matchList) {
  ...
  [...matchList]
    ...
    .forEach((match) => {
      ...
      winner.maxWinStreak = Math.max(winner.maxWinStreak, winner.streak);
...
function buildMajorExtras(matchList) {
  ...
  proleagueGroups.forEach((group) => {
    ...
    group.losers.forEach((playerId) => {
      ...
      if (group.coffee) extras[playerId].coffeeLost += 1;
    });
  });
...
function applyLegacyProleagueExtra(extras, match) {
  ...
  if (match.coffee) {
    extras[match.winner].coffee += 1;
    extras[match.loser].coffeeLost += 1;
  }
}
...
function hydrateControls() {
  ...
  el.mapList.innerHTML = maps
    ...
    .join("");
  if (el.opponentClanList) {
    el.opponentClanList.innerHTML = opponentClans
      .map((clan) => `<option value="${escapeHtml(clan)}"></option>`)
      .join("");
  }
...
function bindEvents() {
  ...
  el.mapPickerResults.addEventListener("copy", copyMapPickerResults);
  ...
  el.addClanwarSet.addEventListener("click", addClanwarSet);
  el.clanwarSets.addEventListener("input", syncDynamicRacePreview);
  el.clanwarSets.addEventListener("click", handleClanwarSetClick);
...
function renderMapPickerOptions() {
  el.mapPickerOptions.innerHTML = maps.map((map, index) => `
    <label class="map-choice">
      <input type="checkbox" value="${escapeHtml(map)}" ${index < 9 ? "checked" : ""}>
...
function renderMapPickerResults(results, emptyText = "뽑기를 누르면 순서대로 표시됩니다.") {
  el.mapPickerResults.innerHTML = results.length
    ? results.map((map, index) => `
      <li class="map-flip-card" data-map-name="${escapeHtml(map)}" data-map-order="${index + 1}" style="--flip-delay: ${index * 260}ms">
...
function copyMapPickerResults(event) {
  const selectedItems = getSelectedMapResultItems();
  if (!selectedItems.length) return;

  event.preventDefault();
  event.clipboardData.setData("text/plain", selectedItems
    .map((item) => `${item.order}. ${item.map}`)
    .join("\n"));
}

function getSelectedMapResultItems() {
  const selection = window.getSelection();
  if (!selection || selection.isCollapsed) return [];

  return [...el.mapPickerResults.querySelectorAll("[data-map-name]")]
    .filter((item) => selectionContainsNode(selection, item))
    .map((item) => ({
      order: item.dataset.mapOrder,
      map: item.dataset.mapName
    }));
}
...
function selectionContainsNode(selection, node) {
  for (let index = 0; index < selection.rangeCount; index += 1) {
    ...
    } else if (selectionContainsNodeFallback(range, node)) {
      return true;
    }
  }
  return false;
...
function selectionContainsNodeFallback(range, node) {
...
function syncRoute() {
  ...
  const routeView = el.views.find((view) => view.dataset.view === route);
  const targetElement = document.getElementById(route);
  const targetView = targetElement?.closest("[data-view]");
  const activeRoute = routeView?.dataset.view || targetView?.dataset.view || "overview";
  ...
  if (!routeView && targetElement && targetView) {
...
function renderLeaderboard() {
  ...
  el.leaderboardBody.innerHTML = filtered.map((player) => {
    ...
    <tr class="leaderboard-row ${tierClass} ${player.rank <= 3 ? `podium-row podium-${player.rank}` : ""} ${player.id === selectedPlayerId ? "selected-row" : ""}" data-player-id="${escapeHtml(player.id)}" tabindex="0">
      ...
      <td>${renderLeaderboardPlayerCell(player)}</td>
...
function renderTierRow(rank, columnCount = getLeaderboardColumnCount()) {
...
function renderSelectedPlayerDetailRow(player) {
  ...
  const seasonHonors = getPlayerSeasonHonorAchievements(player.id);
  return `
        ...
        <section class="player-detail">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-side-stack">
              ...
              <article class="stat-card compact season-honor-card">
                <h4>입상 기록</h4>
                ${renderPlayerSeasonHonorDetails(seasonHonors)}
              </article>
            </div>
            <article class="stat-card compact recent-card">
              <h4>전체 기록 <small>${detail.recent.length}전</small></h4>
...
function renderLeaderboardPlayerCell(player) {
  return `
    <div class="leaderboard-player-cell">
      <button class="player-link" type="button" data-player-id="${escapeHtml(player.id)}">${escapeHtml(player.id)}</button>
      ${renderLeaderboardHonorBadges(player.id)}
    </div>
  `;
}

function renderLeaderboardHonorBadges(playerId) {
  const eloBadges = getSeasonHonorBadges(playerId, majorResults.eloSeasons, "elo");
  const gslBadges = getSeasonHonorBadges(playerId, majorResults.gslSeasons, "gsl");
  if (!eloBadges.length && !gslBadges.length) return "";

  return `
    <div class="leaderboard-honor-badges ${eloBadges.length && gslBadges.length ? "stacked" : "single"}" aria-label="입상 기록">
      ${renderLeaderboardHonorBadgeRow("ELO", eloBadges, Boolean(eloBadges.length && gslBadges.length))}
      ${renderLeaderboardHonorBadgeRow("GSL", gslBadges, Boolean(eloBadges.length && gslBadges.length))}
    </div>
  `;
}

function renderLeaderboardHonorBadgeRow(label, badges, hasMultipleTypes = false) {
  if (!badges.length) return "";

  return `
    <div class="leaderboard-honor-row ${label.toLowerCase()} ${hasMultipleTypes ? "stacked" : ""}">
      <span class="leaderboard-honor-icons">
        ${badges.map((badge) => `
          <span class="leaderboard-honor-badge ${badge.type} rank-${badge.rank}" title="${escapeHtml(badge.label)}">
            ${badge.type === "elo" ? escapeHtml(badge.icon) : ""}
          </span>
        `).join("")}
      </span>
    </div>
...
function getSeasonHonorBadges(playerId, seasons, type) {
    ...
    return results
      ...
      .map((result) => {
        ...
        return {
          type,
          rank,
          icon: getSeasonHonorIcon(type, rank),
          label: `${type === "elo" ? "ELO" : "GSL"} ${season.season || "시즌"} ${rank}위`
        };
      })
      .filter((badge) => Number.isFinite(badge.rank));
  });
}

function getSeasonHonorIcon(type, rank) {
  if (type === "gsl") {
    return "◆";
  }
  if (rank === 1) return "★";
...
function buildPlayerAchievements(playerId) {
  const achievements = [];
  const activeMajorStandings = getActiveStandingsForMode("major");
  const majorStanding = activeMajorStandings.find((player) => player.id === playerId);
  ...
  RACES.forEach((race) => {
    const raceRankings = activeMajorStandings
...
function getPlayerSeasonHonorAchievements(playerId) {
  return [
    ...getConsecutiveFirstPlaceAchievements(playerId),
    ...getSeasonHonorBadges(playerId, majorResults.eloSeasons, "elo")
      .map((badge) => ({
        icon: badge.icon,
        label: badge.label,
        tone: `elo-honor podium-${badge.rank}`
      })),
    ...getSeasonHonorBadges(playerId, majorResults.gslSeasons, "gsl")
      .map((badge) => ({
        icon: badge.icon,
        label: badge.label,
        tone: `gsl-honor podium-${badge.rank}`
      }))
  ];
}

function getConsecutiveFirstPlaceAchievements(playerId) {
  const achievements = [];
  const eloStreak = getMaxConsecutiveFirstPlaceCount(playerId, majorResults.eloSeasons);
  const gslStreak = getMaxConsecutiveFirstPlaceCount(playerId, majorResults.gslSeasons);

  if (eloStreak >= 2) {
    achievements.push({
      icon: "★",
      label: `ELO ${eloStreak}회 연속 1등`,
      tone: "elo-honor podium-1 streak-honor"
    });
  }
  ...
  if (gslStreak >= 2) {
    achievements.push({
      icon: "◆",
      label: `GSL ${gslStreak}회 연속 우승`,
      tone: "gsl-honor podium-1 streak-honor"
    });
  }
...
function getMaxConsecutiveFirstPlaceCount(playerId, seasons) {
  let current = 0;
  let best = 0;

  seasons.forEach((season) => {
    const results = Array.isArray(season.results) ? season.results : [];
    const wonSeason = results.some((result) => Number(result.rank) === 1 && result.playerId === playerId);
    current = wonSeason ? current + 1 : 0;
...
function renderPlayerSeasonHonorDetails(honors) {
  if (!honors.length) return `<p class="empty-state inline-empty">입상 기록이 없습니다.</p>`;

  return `
    <div class="season-honor-list">
      ${honors.map((honor) => `
        <span class="season-honor-chip ${honor.tone}">
          <strong>${escapeHtml(honor.icon)}</strong>
          <span>${escapeHtml(honor.label)}</span>
        </span>
      `).join("")}
    </div>
  `;
...
function getActiveModeRank(playerId, mode) {
  const activeStandings = getActiveStandingsForMode(mode);
  return activeStandings.findIndex((player) => player.id === playerId) + 1 || 0;
}

function getActiveStandingsForMode(mode) {
  return (standingsByMode[mode] || [])
    .filter((player) => hasRecentActivityForMode(player.id, mode))
    .map((player, index) => ({ ...player, rank: index + 1 }));
}
...
function buildHallOfFameEntries() {
  const majorStandings = getActiveStandingsForMode("major");
...
function getHallOfFameCategories(racePlayers, race) {
  ...
  const categories = [
    ...
    { label: "최다 연승", winner: pickBest(eligibleRacePlayers.filter((player) => player.maxWinStreak > 0), (player) => player.maxWinStreak, (player) => `${player.maxWinStreak}연승`) },
...
function createMapRecordSet() {
  return maps.reduce((records, map) => {
    records[map] = { wins: 0, losses: 0 };
    return records;
  }, {});
}
...
function renderMajorExtraCells(player, extrasByPlayer = majorExtrasByPlayer) {
  ...
  return `
    ...
    <td>${renderCoffeeRecord(extras)}</td>
    <td>${formatRecordWithRate(extras.death)}</td>
  `;
}

function renderCoffeeRecord(extras) {
  return `
    <span class="coffee-record">
      <strong>${extras.coffee}잔</strong>
      <small>${extras.coffeeLost ? `-${extras.coffeeLost}잔` : "-0잔"}</small>
    </span>
  `;
}
...
function renderHallOfFame() {
  ...
  el.hallGrid.innerHTML = RACES.map((race) => {
    ...
    const categories = [
      ...
      ["최다 연승", pickBest(eligibleRacePlayers.filter((player) => player.maxWinStreak > 0), (player) => player.maxWinStreak, (player) => `${player.maxWinStreak}연승`)],
...
function renderHonors() {
  renderHonorSeasons(el.eloSeasonHonors, majorResults.eloSeasons, 3);
  renderHonorSeasons(el.gslSeasonHonors, majorResults.gslSeasons, 2, { includeExtraRanks: true });
}

function renderClanWars() {
  if (!el.clanwarSummary || !el.clanwarByClan || !el.clanwarPlayers || !el.clanwarLog) return;

  const seriesList = buildClanWarSeries(clanWarMatches);
  const total = summarizeClanWarSeries(seriesList);
  el.clanwarSummary.innerHTML = `
    <div class="clanwar-score">
      <strong>${total.wins}</strong>
      <span>:</span>
      <strong>${total.losses}</strong>
    </div>
    <p>${formatRecordWithRate(total)}</p>
  `;

  const byClan = summarizeClanWarsByClan(seriesList);
  el.clanwarByClan.innerHTML = byClan.length
    ? byClan.map((entry) => `
      <article class="stat-card compact clanwar-clan-card">
        <h4>${escapeHtml(entry.clan)}</h4>
        <strong>${formatClanWarRecordWithDraws(entry.record)}</strong>
      </article>
    `).join("")
    : `<p class="empty-state inline-empty">클랜전 전적이 없습니다.</p>`;

  const playerRecords = buildClanWarPlayerRecords(clanWarMatches);
  el.clanwarPlayers.innerHTML = playerRecords.length
    ? playerRecords.map(renderClanWarPlayerRow).join("")
    : `<tr><td colspan="9"><p class="empty-state inline-empty">클랜전 참여 기록이 없습니다.</p></td></tr>`;

  el.clanwarLog.innerHTML = seriesList.length
    ? seriesList.sort(compareClanWarSeriesNewestFirst).map(renderClanWarSeriesLogRow).join("")
    : `<p class="empty-state inline-empty">클랜전 경기 기록이 없습니다.</p>`;
}

function buildClanWarPlayerRecords(matchList) {
  const records = new Map();
  const getRecord = (playerId) => {
    const id = canonicalPlayerId(playerId);
    if (!records.has(id)) {
      records.set(id, {
        id,
        race: findPlayer(id)?.race || "",
        wins: 0,
        losses: 0,
        streak: 0,
        vs: createRaceRecordSet(),
        recent: []
      });
    }
  ...
  [...matchList].sort(compareMatchesOldestFirst).forEach((match) => {
    const record = getRecord(match.ourPlayer);
    ...
    const vsRace = RACES.includes(match.opponentRace) ? match.opponentRace : null;
    if (vsRace) {
      if (isWin) record.vs[vsRace].wins += 1;
      else record.vs[vsRace].losses += 1;
    }
    record.recent.unshift(match);
    record.recent = record.recent.slice(0, 10);
  });

  return [...records.values()]
    .filter((record) => recordGames(record) > 0)
    .sort((a, b) => recordGames(b) - recordGames(a) || recordWinRate(b) - recordWinRate(a) || a.id.localeCompare(b.id));
}

function renderClanWarPlayerRow(record, index) {
  const activityRank = index + 1;
  const tierClass = getTierClass(activityRank);
  return `
    <tr class="leaderboard-row clanwar-player-row ${tierClass} ${activityRank <= 3 ? `podium-row podium-${activityRank}` : ""}">
      <td>${renderClanWarActivityBadge(activityRank)}<small>${recordGames(record)}전</small></td>
      <td>${renderLeaderboardPlayerCell(record)}</td>
      <td>${renderRaceMark(record.race)}</td>
      <td>${formatStreak(record.streak)}</td>
      <td>${formatRecordWithRate(record)}</td>
      <td>${formatRecordWithRate(record.vs.Zerg)}</td>
      <td>${formatRecordWithRate(record.vs.Terran)}</td>
      <td>${formatRecordWithRate(record.vs.Protoss)}</td>
      <td><span class="recent-strip">${renderRecentStrip(record.recent.map((match) => match.result))}</span></td>
    </tr>
  `;
}

function renderClanWarActivityBadge(rank) {
  if (rank === 1) return `<span class="rank rank-badge top-1"><span class="rank-medal">🏆</span><span>참여왕</span></span>`;
  return renderRankBadge(rank);
}

function buildClanWarSeries(matchList) {
  const seriesById = new Map();
  matchList.forEach((match, index) => {
    const seriesId = match.seriesId || `legacy-${match.date}-${match.opponentClan}-${match.sourceIndex ?? index}`;
    ...
    series.date = series.date || match.date;
    series.opponentClan = series.opponentClan || match.opponentClan;
    series.sourceIndex = Math.max(series.sourceIndex, getMatchSourceIndex(match, index));
    series.sets.push(match);
  });

  return [...seriesById.values()].map((series) => {
    const wins = series.sets.filter((match) => match.result === "W").length;
    const losses = series.sets.filter((match) => match.result === "L").length;
    return {
      ...series,
      wins,
      losses,
      result: wins > losses ? "W" : losses > wins ? "L" : "D",
      sets: series.sets.sort((a, b) => getMatchSourceIndex(a) - getMatchSourceIndex(b))
    };
  });
}

function compareClanWarSeriesNewestFirst(a, b) {
  return String(b.date || "").localeCompare(String(a.date || "")) || b.sourceIndex - a.sourceIndex;
}

function summarizeClanWarSeries(seriesList) {
  return seriesList.reduce((record, series) => {
    if (series.result === "W") record.wins += 1;
    if (series.result === "L") record.losses += 1;
    return record;
  }, { wins: 0, losses: 0 });
}

function summarizeClanWarsByClan(seriesList) {
  const records = new Map();
  seriesList.forEach((series) => {
    if (!records.has(series.opponentClan)) {
      records.set(series.opponentClan, { clan: series.opponentClan, record: { wins: 0, losses: 0, draws: 0 } });
    }
    const entry = records.get(series.opponentClan);
    if (series.result === "W") entry.record.wins += 1;
    if (series.result === "L") entry.record.losses += 1;
    if (series.result === "D") entry.record.draws += 1;
  });

  return [...records.values()].sort((a, b) => clanWarRecordGames(b.record) - clanWarRecordGames(a.record) || a.clan.localeCompare(b.clan));
}

function clanWarRecordGames(record) {
  return (record.wins || 0) + (record.losses || 0) + (record.draws || 0);
}

function formatClanWarRecordWithDraws(record) {
  const games = clanWarRecordGames(record);
  const rate = games ? Math.round(((record.wins || 0) / games) * 1000) / 10 : 0;
  return `<span class="record-stack"><strong>${record.wins || 0}승 ${record.losses || 0}패 ${record.draws || 0}무</strong><small>${rate}%</small></span>`;
}

function renderClanWarSeriesLogRow(series) {
  const resultLabel = series.result === "W" ? "승" : series.result === "L" ? "패" : "무";
  return `
    <details class="match-card clanwar-match-card ${series.result === "W" ? "win" : series.result === "L" ? "loss" : "draw"}">
      <summary>
        <div class="match-meta">
          <span>${escapeHtml(series.date)}</span>
          <strong>${escapeHtml(series.opponentClan)} · ${resultLabel}</strong>
        </div>
        <p>${series.wins}:${series.losses} (${series.sets.length}세트)</p>
      </summary>
      <div class="deathmatch-set-detail">
        ${series.sets.map((match, index) => {
          const ourPlayer = findPlayer(match.ourPlayer);
          const setResultLabel = match.result === "W" ? "승" : "패";
          return `
            <div class="deathmatch-set-row">
              <span>${index + 1}세트</span>
              <strong>${escapeHtml(match.ourPlayer)} ${setResultLabel} vs ${escapeHtml(match.opponentPlayer)}</strong>
              <small>${escapeHtml(ourPlayer?.race || "-")} vs ${escapeHtml(match.opponentRace || "-")}</small>
            </div>
          `;
        }).join("")}
      </div>
...
function renderClanWarLogRow(match) {
  const ourPlayer = findPlayer(match.ourPlayer);
  const resultLabel = match.result === "W" ? "승" : "패";
  return `
    <article class="match-card clanwar-match-card ${match.result === "W" ? "win" : "loss"}">
      <div class="match-meta">
        <span>${escapeHtml(match.date)}</span>
        <strong>${escapeHtml(match.opponentClan)} · ${resultLabel}</strong>
      </div>
      <p><strong>${escapeHtml(match.ourPlayer)}</strong> vs ${escapeHtml(match.opponentPlayer)}</p>
      <p>${ourPlayer?.race || "-"} vs ${escapeHtml(match.opponentRace || "-")}</p>
    </article>
  `;
}

function renderHonorSeasons(container, seasons, expectedRanks, options = {}) {
  if (!container) return;
  if (!seasons.length) {
    container.innerHTML = `<p class="empty-state inline-empty">기록된 성적이 없습니다.</p>`;
    return;
  }

  container.innerHTML = seasons.map((season) => renderHonorSeasonCard(season, expectedRanks, options)).join("");
}

function renderHonorSeasonCard(season, expectedRanks, options = {}) {
  const resultByRank = new Map((Array.isArray(season.results) ? season.results : [])
    .map((result) => [Number(result.rank), result]));
  const requiredRanks = Array.from({ length: expectedRanks }, (_, index) => index + 1);
  const extraRanks = options.includeExtraRanks
    ? [...resultByRank.keys()].filter((rank) => rank > expectedRanks).sort((a, b) => a - b)
    : [];
  const ranks = [...requiredRanks, ...extraRanks];

  return `
    <article class="honor-season-card">
      <div class="honor-season-head">
        <strong>${escapeHtml(season.season || "시즌")}</strong>
        ${season.note ? `<small>${escapeHtml(season.note)}</small>` : ""}
      </div>
      <div class="honor-winner-list">
        ${ranks.map((rank) => renderHonorWinner(rank, resultByRank.get(rank))).join("")}
      </div>
    </article>
  `;
}

function renderHonorWinner(rank, result) {
  const playerId = `${result?.playerId || ""}`.trim();
  const player = playerId ? findPlayer(playerId) : null;
  const raceClass = player?.race ? player.race.toLowerCase() : "";
  const displayRace = player?.race || result?.race || "미등록";

  return `
    <div class="honor-winner top-commander-card ${raceClass} podium-${rank}">
      <div class="commander-image"></div>
      <div class="commander-content">
        ${renderRankBadge(rank)}
        <strong>${playerId ? escapeHtml(playerId) : "-"}</strong>
        <small>${escapeHtml(displayRace)}</small>
      </div>
    </div>
  `;
}
...
function renderMapRaceStats() {
  ...
  el.mapRaceStats.innerHTML = mapNames.length ? mapNames.map((map) => `
    <article class="stat-card">
      ...
      <div class="stat-list">
        ${Object.values(byMap[map]).sort(compareRaceMatchupRecords).map((record) => `
          <div class="stat-row">
...
function getRaceMatchupKey(raceA, raceB) {
  if (raceA === raceB) return `${raceA} vs ${raceB}`;

  const pairKey = [raceA, raceB].sort().join("|");
  const matchupOrder = {
    "Terran|Zerg": "Zerg vs Terran",
    "Protoss|Terran": "Terran vs Protoss",
    "Protoss|Zerg": "Protoss vs Zerg"
  };
  return matchupOrder[pairKey] || `${raceA} vs ${raceB}`;
}

function compareRaceMatchupRecords(a, b) {
  return getRaceMatchupDisplayOrder(a.key) - getRaceMatchupDisplayOrder(b.key)
    || a.key.localeCompare(b.key);
}

function getRaceMatchupDisplayOrder(key) {
  const order = {
    "Zerg vs Terran": 1,
    "Zerg vs Zerg": 2,
    "Terran vs Protoss": 3,
...
function formatRaceMatchupRecord(record) {
  ...
  const sides = races.map((race) => {
    const wins = record.wins[race] || 0;
    const losses = record.total - wins;
    const rate = record.total ? wins / record.total : 0;
    return { race, wins, losses, rate };
  });
  const [first, second] = sides;

  if (first.wins === second.wins) {
    return `<span class="matchup-side even">${first.wins}승 ${first.losses}패 · ${formatPercent(first.rate)} · 동률</span>`;
  }

  const leader = first.wins > second.wins ? first : second;
  return `<span class="matchup-side advantage ${leader.race.toLowerCase()}">${leader.race} ${leader.wins}승 ${leader.losses}패 · ${formatPercent(leader.rate)} · 유리</span>`;
}
...
function compareRecentMatches(a, b) {
  return compareMatchesNewestFirst(a, b);
}

function compareMatchesNewestFirst(a, b) {
  return compareMatchDates(b, a) || getMatchSourceIndex(b) - getMatchSourceIndex(a);
}

function compareMatchesOldestFirst(a, b) {
  return compareMatchDates(a, b) || getMatchSourceIndex(a) - getMatchSourceIndex(b);
}
...
function syncTypeFields() {
  ...
  const isClanwar = el.matchType.value === "clanwar";
  el.singleMatchFields.hidden = isProleague || isDeathmatch || isClanwar;
  el.proleagueBatch.hidden = !isProleague;
  el.deathmatchBatch.hidden = !isDeathmatch;
  el.clanwarBatch.hidden = !isClanwar;
}
...
function syncDynamicRacePreview(event) {
  ...
  const key = input.dataset.normal || input.dataset.proleague || input.dataset.clanwar;
  if (!key || key === "map") return;

  const row = input.closest(".normal-set, .proleague-set, .clanwar-set");
...
function renderClanwarSets(count) {
  el.clanwarSets.innerHTML = "";
  for (let index = 0; index < count; index += 1) {
    el.clanwarSets.insertAdjacentHTML("beforeend", renderClanwarSet(index + 1));
  }
}

function renderClanwarSet(number) {
  return `
    <div class="clanwar-set" data-set-number="${number}">
      <div class="set-number">${number}</div>
      <label class="field player-field">
        <span>우리 선수</span>
        <div class="input-with-race">
          <div class="clearable-input">
            <input data-clanwar="ourPlayer" list="player-list" autocomplete="off" placeholder="우리 선수 ID">
            <button class="clear-input" type="button" aria-label="우리 선수 입력 지우기">X</button>
          </div>
          <span class="race-preview" data-race-preview-for="ourPlayer">종족</span>
        </div>
      </label>
      <label class="field">
        <span>상대 선수</span>
        <div class="clearable-input">
          <input data-clanwar="opponentPlayer" autocomplete="off" placeholder="상대 선수 ID">
          <button class="clear-input" type="button" aria-label="상대 선수 입력 지우기">X</button>
        </div>
      </label>
      <label class="field">
        <span>상대 종족</span>
        <select data-clanwar="opponentRace">
          ${RACES.map((race) => `<option value="${race}">${race}</option>`).join("")}
        </select>
      </label>
      <label class="field">
        <span>결과</span>
        <select data-clanwar="result">
...
function addClanwarSet() {
  el.clanwarSets.insertAdjacentHTML("beforeend", renderClanwarSet(getClanwarRows().length + 1));
  renumberRows(getClanwarRows());
}

function handleClanwarSetClick(event) {
  if (event.target.closest(".remove-set")) {
    const rows = getClanwarRows();
    if (rows.length <= 1) return;
    event.target.closest(".clanwar-set").remove();
    renumberRows(getClanwarRows());
    return;
  }
...
function getClanwarRows() {
  return [...el.clanwarSets.querySelectorAll(".clanwar-set")];
}
...
function validatePayload(payload) {
  ...
  if (payload.type === "clanwarBatch") return validateClanwarPayload(payload);
...
function validateProleaguePayload(payload) {
  ...
  if (!proleagueTeamWinnerMatchesSetScore(payload)) {
    return "프로리그 팀 승리 선택과 세트 승자 수가 맞지 않습니다.";
  }
...
function buildClanwarPayload() {
  const opponentClan = el.clanwarOpponentClan.value.trim();
  const seriesId = createSubmissionId("clanwar");
  const matches = getClanwarRows()
    .map((row) => {
      const ourPlayer = row.querySelector("[data-clanwar='ourPlayer']").value.trim();
      const opponentPlayer = row.querySelector("[data-clanwar='opponentPlayer']").value.trim();
      const opponentRace = row.querySelector("[data-clanwar='opponentRace']").value;
      const result = row.querySelector("[data-clanwar='result']").value;
      if (!ourPlayer && !opponentPlayer) return null;
...
function validateClanwarPayload(payload) {
  if (!payload.date) return "날짜를 입력하세요.";
  if (!payload.opponentClan) return "상대 클랜을 입력하세요.";
  if (opponentClans.length && !opponentClans.some((clan) => clan.toLowerCase() === payload.opponentClan.toLowerCase())) {
    return "상대 클랜은 등록된 클랜 목록에서 선택해야 합니다.";
  }
  if (!payload.matches.length) return "클랜전 경기를 최소 1개 입력하세요.";

  for (const [index, match] of payload.matches.entries()) {
    const gameNo = index + 1;
    if (!findPlayer(match.ourPlayer)) return `${gameNo}경기 우리 선수는 선수 목록에 있는 ID여야 합니다.`;
    if (!match.opponentPlayer) return `${gameNo}경기 상대 선수 ID를 입력하세요.`;
    if (!RACES.includes(match.opponentRace)) return `${gameNo}경기 상대 종족을 선택하세요.`;
    if (match.result !== "W" && match.result !== "L") return `${gameNo}경기 승리 여부를 선택하세요.`;
  }
...
async function submitToSheets(payload) {
  ...
  try {
    ...
    if (payload.type === "clanwarBatch") {
      clanWarMatches = normalizeClanWarMatches([...clanWarMatches, ...payload.matches]);
      renderClanWars();
    }
    el.message.textContent = payload.type === "clanwarBatch"
      ? "클랜전 전적이 접수되었습니다."
      : payload.type === "proleagueBatch" || payload.type === "deathmatchBatch"

  ...
  el.clanwarByClan.innerHTML = byClan.length
    ? byClan.map((entry) => `
      <article class="stat-card compact clanwar-clan-card">
        ...
        <strong>${formatRecordWithRate(entry.record)}</strong>
      </article>
    `).join("")
...
function summarizeClanWarsByClan(seriesList) {
  ...
  seriesList.forEach((series) => {
    if (!records.has(series.opponentClan)) {
      records.set(series.opponentClan, { clan: series.opponentClan, record: { wins: 0, losses: 0 } });
    }
  ...
  return [...records.values()].sort((a, b) => recordGames(b.record) - recordGames(a.record) || a.clan.localeCompare(b.clan));
}

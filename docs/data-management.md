...
The website now reads `Players`, `Maps`, `ApprovedMatches`, `EloSeasonHonors`, `GslSeasonHonors`, `OpponentClans`, and `ClanWarMatches` directly from the Apps Script Web App when `src/config.js` has:
...
## Major Results Sheets

Run `setupEloBoardSheets` after updating the Apps Script. It creates two result sheets:

- `EloSeasonHonors`: season ELO podiums. Use ranks 1, 2, and 3.
- `GslSeasonHonors`: GSL podiums. Use ranks 1 and 2. Rank 3 is also displayed if entered.

Both sheets use the same columns:

- `season`: for example `시즌 1` or `GSL 시즌 1`
- `rank`: placement number
- `playerId`: player ID from the `Players` sheet
- `note`: optional season note

Example `EloSeasonHonors` rows:

```txt
season | rank | playerId | note
시즌 1 | 1 | GG_player1 |
시즌 1 | 2 | GG_player2 |
시즌 1 | 3 | GG_player3 |
```

Example `GslSeasonHonors` rows:

```txt
season | rank | playerId | note
GSL 시즌 1 | 1 | GG_player1 |
GSL 시즌 1 | 2 | GG_player2 |
```

## Clan War Sheets

Run `setupEloBoardSheets` after updating the Apps Script. It creates two clan war sheets:

- `OpponentClans`: opponent clan names used by the clan war input form.
- `ClanWarMatches`: submitted clan war match records. These do not affect ELO.

`OpponentClans` columns:

- `clan`: opponent clan name

`ClanWarMatches` columns:
...
- `opponentPlayer`
- `opponentRace`
- `result`: `W` or `L` from our clan perspective
- `submittedAt`
- `seriesId`: shared by rows submitted together, used to group one clan war series
